...
-(void)setData:(int)index dictForChat:(XmppFriend *)xmppFriend
{
    if (xmppFriend.friend_DisplayName.length)
        self.lblNameCell.text = xmppFriend.friend_DisplayName;
    else
        self.lblNameCell.text = xmppFriend.friend_Name;
    
    NSLog( @"MsgCount %d",[xmppFriend.messageCount intValue]);
    
    if ([xmppFriend.messageCount intValue])
    {
        self.lblMsgCounter.text = [NSString stringWithFormat:@"%d",[xmppFriend.messageCount intValue]];
    }
    else
...
-(void)setUIForBlockedUser:(XmppFriend *)xmppFriend
{
