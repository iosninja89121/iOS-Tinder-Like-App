...
@interface ChatViewController ()<MomentVCDelegate,CaptureMomentVCDelegate>
{
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter ]addObserver:self selector:@selector(getMessageNotification:) name:NOTIFICATION_XMPP_UPDATE_MESSAGE_COUNTER object:nil];
...
- (void)resetFrameForContainerView
{
    ...
    if (arrayMoments.count == 0)
    {
        ...
        {
             tblView.frame = CGRectMake(0, SearchBarHeight, ScreenSize.width, ScreenSize.height-44);
...
-(void)pushToChatViewWithXmppFriend:(XmppFriend *)xmppFriend
{
